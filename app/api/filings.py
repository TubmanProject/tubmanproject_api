...
from app.filings import Filings
...
filings = Blueprint('filings', __name__, url_prefix='/v1/filings')


@filings.route('/', methods=['GET'])
def get_filing_data():
    """Get filing data."""
    ...
    try:
        ...
        cursor, metadata = Filings.find(request_args)
    ...
    try:
        if total_docs > 0:
            cache_mongodb_request_task(Filings, request_args, encoded_full_path, data['metadata'])
    except Exception as err:
