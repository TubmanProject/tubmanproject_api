...
from app.filings.schema import filings_json_schema, filings_denormalized_pipeline
...
class Filings(object):
    ...
    Establish the schema for filing objects.
    Define methods for downloading and parsing filing data.
    """

    schema_validator = filings_json_schema
    ...
    def setup_schema(cls):
        ...
        db.create_collection('filings', validator=cls.schema_validator)

    @classmethod
    def create_denormalized_view(cls):
        """Create a denormalized view of the model."""
        db.command('create', 'denormalized_filings_view',
                   viewOn='filings',
                   pipeline=filings_denormalized_pipeline)

    @classmethod
    def find(cls, request_params):
        """Find filings from an http request.
        ...
        param_map = {
            ...
            'defendant_birthplace': {
                'path': 'defendant.birthplace.definition',
                'method': string_search
            },
            'defendant_uscitizen': {
                'path': 'defendant.us_citizen.definition',
                'method': string_search
            },
            'attorney_spn': {
                'path': 'attorney.spn',
        ...
        if not request_params:
            cursor = db.denormalized_filings_view.find(filter=query,
                                                       limit=100,
                                                       batch_size=100)
